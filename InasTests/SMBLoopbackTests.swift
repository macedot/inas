...
final class SMBLoopbackTests: XCTestCase {
    ...
    func testTransferVerify() throws {
        let port = try startServer()
        var err = [CChar](repeating: 0, count: 256)
        XCTAssertEqual(
            inas_smb_client_transfer_verify("127.0.0.1", port, "inas", "correct1", "inas", &err, Int32(err.count)),
            0,
            String(cString: err)
        )
    }

    func testParallelCreateDeleteList() throws {
        let port = try startServer()
        var err = [CChar](repeating: 0, count: 256)
        XCTAssertEqual(
            inas_smb_client_parallel_create_delete_list("127.0.0.1", port, "inas", "correct1", "inas", &err, Int32(err.count)),
            0,
            String(cString: err)
        )
    }

    func testFlushIsFast() throws {
        let port = try startServer()
        var err = [CChar](repeating: 0, count: 256)
        XCTAssertEqual(
            inas_smb_client_flush_is_fast("127.0.0.1", port, "inas", "correct1", "inas", &err, Int32(err.count)),
            0,
            String(cString: err)
        )
    }

    func testTwoSessionsCount() throws {
        let port = try startServer()
        var err = [CChar](repeating: 0, count: 256)
        XCTAssertEqual(
            inas_smb_client_two_sessions_count("127.0.0.1", port, "inas", "correct1", "inas", &err, Int32(err.count)),
            0,
            String(cString: err)
        )
    }
    ...
    func testStreamsRoundTrip() throws {
        let port = try startServer()
        var err = [CChar](repeating: 0, count: 256)
        XCTAssertEqual(
            inas_smb_client_stream_roundtrip("127.0.0.1", port, "inas", "correct1", "inas", &err, Int32(err.count)),
            0,
            String(cString: err)
        )
    }
